...
function getBook (){
    ...
    .then(([booksData, authorsData]) => {
        ...
        bookbox.addEventListener('input', e => {  
            const searchQuery = e.target.value.trim().toLowerCase()
            filterBooks(searchQuery)
        })
         ...
         // Function to filter books based on search query
function filterBooks(searchQuery) {
    // Clear existing book elements before creating new ones
    bookContainer.innerHTML = ''

    books.forEach(book => {
        const titleMatch = book.title.toLowerCase().includes(searchQuery);
        const authorMatch = authorsData.find(author => author._id === book.author)?.name.toLowerCase().includes(searchQuery)
        const isVisible = titleMatch || authorMatch

        if (isVisible) {
            // Creating HTML elements for matching books only
            const bookDiv = document.createElement('div')
            const title = document.createElement('ul')
            const image = document.createElement('img')
            ...
            // Append the book container to the group container
            bookDiv.classList.add('book1') // Use a class for styling
